...
static int run()
{
	...
	for (int i = 0; i < cmdcnt; i++) {
		...
		if (cmd == NEWACCOUNT) {
			...
			if (result != param1) {
				ret = 0;
				printf("ERROR at TICK(%d), must be(%d) but got (%d)\n", tick_now, param1, result);
			} else {
				//printf("GOOD! at TICK(%d), must be(%d) but got (%d)\n", tick_now, param1, result);
			}
		}
	}
...
//-------------------------------------------------------------------------------------
    ...
    struct t_data {
		...
		void initialize(char in_id[11], char in_password[11], int in_defaulttime, int tickTime) {
			...
			printf("at (%d) initializing id(%s) pass(%s) deftime(%d) nexttick(%d)\n", tickTime, id, password, defaultTime, nextTick);
		}
	...
	t_minHeap extractMinHeap() {
		...
		//printf("extractMinHeap at tick(%d) got id(%s) nextTick(%d)\n", now, data[root.index].id, data[root.index].nextTick);
