// from : http://www.geeksforgeeks.org/largest-subarray-with-equal-number-of-0s-and-1s/
...
int findSubArray(int arr[], int n)
{
    ...
    // sumleft[i] will be sum of array elements from arr[0] to arr[i]
    int sumleft[n];
    ...
    // Now calculate the max value of j - i such that sumleft[i] = sumleft[j]. The idea is 
    // to create a hash table to store indexes of all visited values.   
    // If you see a value again, that it is a case of sumleft[i] = sumleft[j]. Check if this j-i is more than maxsize. 
    // The optimum size of hash will be max-min+1 as these many different values of sumleft[i] are
    // possible. Since we use optimum size, we need to shift all values in sumleft[] by min before 
